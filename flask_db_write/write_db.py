...
class DeleteForm(FlaskForm):
    id_field = HiddenField()
    purpose = HiddenField()
    submit = SubmitField('Eliminar el Registro')
...
def add_record():
    ...
    if form1.validate_on_submit():
        ...
        record = Sock(nombre, apellido, email)
...
def select_record(letters):
    ...
    a, b = list(letters)
    socks = Sock.query.filter(Sock.nombre.between(a, b)).order_by(Sock.nombre).all()
...
def edit_result():
    ...
    sock = Sock.query.filter(Sock.id == id).first()
    # update all values
    sock.nombre = request.form['nombre']
    sock.apellido = request.form['apellido']
    sock.email = request.form['email']
    # get today's date from function, above all the routes
